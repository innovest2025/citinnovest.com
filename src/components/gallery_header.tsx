...
type EventKey = keyof typeof eventsData;
type EventType = {
  title: string;
  icon: JSX.Element;
  color: string;
...
const eventsData: Record<string, EventType> = {
  workshop: {
    ...
    images: [w1, w2, w3, w4], // Imported images for workshop
...
const events: Record<EventKey, EventType> = eventsData;
...
const EventsGallery = () => {
  ...
  }, [selectedEvent, currentImageIndex, isAutoPlay]);

  const handleEventClick = (eventKey: EventKey): void => {
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center p-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl w-full">
            {Object.entries(events).map(([key, event], index) => (
              ...
              >
                <div className={`${event.gradient} p-8 rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-500 relative overflow-hidden`}>
